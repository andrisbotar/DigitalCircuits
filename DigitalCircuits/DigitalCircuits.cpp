...
void example_circuit(Circuit& circuit1) {
    ...
    circuit1.addcomponent(std::move(std::make_unique< NOTgate>(new NOTgate(1, 7))));
    circuit1.addcomponent(std::move(std::make_unique< buffer>(new buffer(1, 7))));
    circuit1.addcomponent(std::move(std::make_unique< Majorityfunction>(new Majorityfunction(1, 2, 7))));
}
